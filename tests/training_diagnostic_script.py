...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('training_diagnostic.log'),
...
class TrainingDiagnostic:
    ...
    def setup_test_environment(self):
        ...
        logger.info(f"Using checkpoint from episode {episode_num}")
        ...
        for ext in ['.json', '.pt']:
            ...
            if source_file.exists():
                ...
                logger.info(f"Copied {source_file.name} to test environment")
    ...
    def load_checkpoint(self):
        ...
        logger.info("Loading checkpoint for training...")
        
        try:
            # Debug: Check what checkpoints are actually in the test directory
            test_checkpoints_dir = self.test_dir / "checkpoints"
            logger.info(f"Test checkpoints directory: {test_checkpoints_dir}")
            if test_checkpoints_dir.exists():
                files = list(test_checkpoints_dir.glob("*"))
                logger.info(f"Files in test checkpoints directory: {[f.name for f in files]}")
            else:
                logger.error("Test checkpoints directory does not exist!")
        ...
        while self.monitoring_active:
            try:
                ...
                if current_episode != self.current_episode:
                    ...
                    self.last_progress_time = time.time()
                    logger.info(f"Training progress: Episode {current_episode}")
                
                # Check for timeout
                if time.time() - self.last_progress_time > self.progress_timeout:
                    logger.error(f"Training stalled at episode {current_episode} for {self.progress_timeout} seconds")
                    self.perform_stall_diagnostics()
                    break
                
                # Check if training stopped unexpectedly
                if not is_training and self.training_started:
                    logger.error("Training stopped unexpectedly")
                    self.perform_stall_diagnostics()
                    break
                ...
                if current_episode >= 100 and not self.checkpoint_created:
                    ...
                    if response.status_code == 200:
                        ...
                        if new_checkpoints:
                            logger.info(f"Checkpoint created at episode {current_episode}")
                ...
                if current_episode >= 100:
                    logger.info("Reached episode 100 - training diagnostic complete")
                    break
    ...
    def perform_stall_diagnostics(self):
        ...
        logger.info("Performing stall diagnostics...")
        ...
        try:
            ...
            if self.backend_process and self.backend_process.pid:
                ...
                logger.info(f"Backend process - Memory: {memory_info.rss / 1024 / 1024:.1f}MB, CPU: {cpu_percent:.1f}%")
            else:
                logger.warning("Backend process not available for resource monitoring")
            ...
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    logger.info(f"GPU Memory - Allocated: {gpu_memory:.1f}MB, Reserved: {gpu_memory_reserved:.1f}MB")
            except ImportError:
                logger.info("PyTorch not available for GPU memory check")
            
            # Get backend logs
            if self.backend_process and self.backend_process.stdout:
                logger.info("Recent backend output:")
                # This is a simplified approach - in practice you'd want to capture logs continuously
                logger.info("Backend process is still running")
            
        except Exception as e:
            logger.error(f"Error during stall diagnostics: {e}")
    
    def run_diagnostic(self):
        """Run the complete diagnostic process"""
        logger.info("Starting training diagnostic...")
...
def main():
    """Main entry point"""
    print("=" * 60)
    print("2048 Bot Training Diagnostic Script")
    print("=" * 60)
    ...
    try:
        ...
        if success:
            print("✅ Training diagnostic completed successfully")
            print(f"✅ Reached episode {diagnostic.current_episode}")
            if diagnostic.checkpoint_created:
                print("✅ Checkpoint creation verified")
            else:
                print("⚠️  No new checkpoint created (may be normal)")
        else:
            print("❌ Training diagnostic failed")
            print(f"❌ Stopped at episode {diagnostic.current_episode}")
        
        print()
        print("Check 'training_diagnostic.log' for detailed logs")
