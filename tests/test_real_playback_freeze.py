...
class RealPlaybackFreezeTest:
    ...
    async def test_real_checkpoint_playback(self, checkpoint_id: str):
        ...
        try:
            ...
            if 'error' in game_result:
                print(f"❌ Single game failed: {game_result['error']}")
                return False, False
            ...
            try:
                ...
                print(f"✅ Live playback completed in {playback_time:.2f}s")
                return True, False
            except asyncio.TimeoutError:
                playback_time = time.time() - start_time
                print(f"⏰ Live playback timed out after {playback_time:.2f}s - potential freeze!")
                self.playback.stop_playback()
                try:
                    await asyncio.wait_for(playback_task, timeout=5.0)
                    print("✅ Playback stopped gracefully")
                except asyncio.TimeoutError:
                    print("❌ Playback failed to stop gracefully - confirmed freeze!")
                return False, True
    ...
    async def test_multiple_checkpoints(self):
        ...
        print("🔍 Testing Multiple Checkpoints for Freezing Issues")
        print("=" * 60)
        ...
        for checkpoint in checkpoints[:3]:  # Test first 3 checkpoints
            ...
            try:
                ...
                print(f"⏰ Entire test timed out after {self.test_timeout} seconds - potential freeze!")
        ...
        if frozen_count > 0:
            print(f"\n🔍 Found {frozen_count} checkpoint(s) that caused freezing!")
        else:
            print(f"\n✅ No freezing detected in checkpoint tests")
    async def test_websocket_stress(self):
        ...
        try:
            ...
            duration = time.time() - start_time
            print(f"❌ Stress test failed after {duration:.2f}s: {e}")
    async def run_all_tests(self):
        """Run all real playback tests"""
        print("🧪 Starting Real Playback Freeze Tests")
        print("=" * 60)
